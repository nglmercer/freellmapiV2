...
import { sql, eq, and, max, asc } from 'drizzle-orm';
...
export function runInTransaction<T>(cb: (tx: any) => T): T {
  return (db as any).transaction(cb);
}
...
export function initDb(dbPath?: string): BunSQLiteDatabase<typeof schema> {
  ...
  createTables(sqlite);
  initEncryptionKey(sqlite as any);
  seedModels();
  migrateModels();
  migrateModelsV2();
  migrateModelsV3Ranks();
  migrateModelsV4();
  migrateModelsV5();
  migrateModelsV6();
  migrateModelsV7();
...
function seedModels() {
  const result = db.select({ count: sql<number>`count(*)` }).from(schema.models).get();
  ...
  db.transaction(() => {
    db.insert(schema.models).values(modelsData).run();

    const allModels = db.select({ id: schema.models.id, intelligenceRank: schema.models.intelligenceRank })
      .from(schema.models)
    ...
    db.insert(schema.fallbackConfig).values(fallbackData).run();
  });
...
function migrateModels() {
  db.transaction(() => {
    ...
    db.update(schema.models).set({ rpdLimit: 20, monthlyTokenBudget: '~3M' }).where(and(eq(schema.models.platform, 'google'), eq(schema.models.modelId, 'gemini-2.5-flash'))).run();
    db.update(schema.models).set({ rpmLimit: 20 }).where(and(eq(schema.models.platform, 'sambanova'), eq(schema.models.modelId, 'Meta-Llama-3.3-70B-Instruct'))).run();
    db.update(schema.models).set({ tpmLimit: 6000 }).where(and(eq(schema.models.platform, 'groq'), eq(schema.models.modelId, 'llama-4-scout-17b-16e-instruct'))).run();
    db.update(schema.models).set({ monthlyTokenBudget: '~1-2M' }).where(and(eq(schema.models.platform, 'cohere'), eq(schema.models.modelId, 'command-r-plus-08-2024'))).run();
    db.update(schema.models).set({ monthlyTokenBudget: '~1-3M' }).where(and(eq(schema.models.platform, 'huggingface'), eq(schema.models.modelId, 'accounts/fireworks/models/llama-v3p3-70b-instruct'))).run();
    db.update(schema.models).set({ monthlyTokenBudget: 'credits-based', enabled: 0 }).where(and(eq(schema.models.platform, 'nvidia'), eq(schema.models.modelId, 'meta/llama-3.1-70b-instruct'))).run();
    ...
    for (const m of newModels) {
      db.insert(schema.models).values(m).onConflictDoNothing().run();
    }

    const missing = db.select({ id: schema.models.id })
      .from(schema.models)
      .leftJoin(schema.fallbackConfig, eq(schema.models.id, schema.fallbackConfig.modelDbId))
      .where(sql`${schema.fallbackConfig.id} IS NULL`)
      .orderBy(asc(schema.models.intelligenceRank))
      .all();

    if (missing.length > 0) {
      const maxPriorityResult = db.select({ mx: max(schema.fallbackConfig.priority) }).from(schema.fallbackConfig).get();
...
function migrateModelsV2() {
  db.transaction(() => {
    ...
    for (const { platform, modelId } of removals) {
      const model = db.select({ id: schema.models.id }).from(schema.models).where(and(eq(schema.models.platform, platform), eq(schema.models.modelId, modelId))).get();
      if (model) {
    ...
    for (const a of additions) {
      db.insert(schema.models).values(a).onConflictDoNothing().run();
    }
...
function migrateModelsV3Ranks() {
  ...
  db.transaction(() => {
    for (const [rank, platform, modelId] of ranks) {
      db.update(schema.models).set({ intelligenceRank: rank }).where(and(eq(schema.models.platform, platform), eq(schema.models.modelId, modelId))).run();
    }
  });
}

function migrateModelsV4() {
  db.transaction(() => {
    const removals = [
    ...
    db.update(schema.models).set({ tpmLimit: 12000 }).where(and(eq(schema.models.platform, 'groq'), eq(schema.models.modelId, 'llama-3.3-70b-versatile'))).run();
    db.update(schema.models).set({ rpdLimit: 20 }).where(and(eq(schema.models.platform, 'sambanova'), eq(schema.models.modelId, 'Meta-Llama-3.3-70B-Instruct'))).run();
    db.update(schema.models).set({ rpdLimit: 14400 }).where(and(eq(schema.models.platform, 'cerebras'), eq(schema.models.modelId, 'qwen-3-235b-a22b-instruct-2507'))).run();
    db.update(schema.models).set({ rpdLimit: 250, monthlyTokenBudget: '~25M' }).where(and(eq(schema.models.platform, 'google'), eq(schema.models.modelId, 'gemini-2.5-flash'))).run();
    db.update(schema.models).set({ rpdLimit: 50, monthlyTokenBudget: '~6M' }).where(and(eq(schema.models.platform, 'google'), eq(schema.models.modelId, 'gemini-2.5-pro'))).run();
    ...
    for (const [r, p, m] of ranks) {
      db.update(schema.models).set({ intelligenceRank: r }).where(and(eq(schema.models.platform, p), eq(schema.models.modelId, m))).run();
    }
  });
}

function migrateModelsV5() {
  db.transaction(() => {
    db.update(schema.models).set({ enabled: 0 }).where(and(eq(schema.models.platform, 'google'), eq(schema.models.modelId, 'gemini-2.5-pro'))).run();
    db.insert(schema.models).values({ platform: 'cerebras', modelId: 'zai-glm-4.7', displayName: 'GLM-4.7 (Cerebras)', intelligenceRank: 7, speedRank: 1, sizeLabel: 'Frontier', rpmLimit: 10, rpdLimit: 100, tpmLimit: null, tpdLimit: null, monthlyTokenBudget: '~3M', contextWindow: 8192 }).onConflictDoNothing().run();
...
function migrateModelsV6() {
  db.transaction(() => {
    const removals = [
      ['openrouter', 'arcee-ai/trinity-large-preview:free'],
    ] as const;
    for (const [p, m] of removals) {
      const model = db.select({ id: schema.models.id }).from(schema.models).where(and(eq(schema.models.platform, p), eq(schema.models.modelId, m))).get();
      if (model) {
        db.delete(schema.fallbackConfig).where(eq(schema.fallbackConfig.modelDbId, model.id)).run();
        db.delete(schema.models).where(eq(schema.models.id, model.id)).run();
      }
    ...
    db.update(schema.models).set({ rpdLimit: 20, monthlyTokenBudget: '~3M' }).where(and(eq(schema.models.platform, 'google'), eq(schema.models.modelId, 'gemini-2.5-flash'))).run();
    db.update(schema.models).set({ rpdLimit: 20, monthlyTokenBudget: '~3M' }).where(and(eq(schema.models.platform, 'google'), eq(schema.models.modelId, 'gemini-2.5-flash-lite'))).run();
    ...
    for (const a of additions) db.insert(schema.models).values(a).onConflictDoNothing().run();
...
function migrateModelsV9() {
  db.update(schema.models).set({ enabled: 0 }).where(and(eq(schema.models.platform, 'cerebras'), eq(schema.models.modelId, 'zai-glm-4.7'))).run();
}

function migrateModelsV10() {
  db.transaction(() => {
    const additions = [
...
function migrateModelsV11() {
  db.transaction(() => {
    db.update(schema.models).set({ modelId: 'qwen-3-235b-a22b-instruct-2507' }).where(and(eq(schema.models.platform, 'cerebras'), eq(schema.models.modelId, 'qwen3-235b'))).run();
    db.update(schema.models).set({ enabled: 1, monthlyTokenBudget: '~3M (1k credits)' }).where(and(eq(schema.models.platform, 'nvidia'), eq(schema.models.modelId, 'meta/llama-3.1-70b-instruct'))).run();
...
function ensureUnifiedKey() {
  const existing = db.select({ value: schema.settings.value }).from(schema.settings).where(eq(schema.settings.key, 'unified_api_key')).get();
  if (!existing) {
    const key = `freellmapi-${crypto.randomBytes(24).toString('hex')}`;
    db.insert(schema.settings).values({ key: 'unified_api_key', value: key }).run();
